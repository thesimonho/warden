...
| File          | Purpose                                                                                                                                                                                                                                                                                                                                                             |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client.go`   | `Client` interface — the key architectural boundary. Each method maps 1:1 to an API endpoint with GoDoc comment. Methods: list/add/remove projects, reset costs, purge audit, list/create/connect worktrees, terminal operations (including `AttachTerminal(ctx, projectID, agentType, worktreeID)` for agent terminal and `AttachShellTerminal(ctx, projectID, agentType, worktreeID)` for auxiliary bash shell), settings, event log, defaults, event subscription.            |
| `adapter.go`  | `ServiceAdapter` wraps `*Warden` to satisfy `Client` for embedded mode (TUI binary). Most methods delegate to `w.Service`; `AttachTerminal(ctx, projectID, agentType, worktreeID)` and `AttachShellTerminal(ctx, projectID, agentType, worktreeID)` use shared `attachTmuxExec()` helper with different session names; `SubscribeEvents` uses `w.Broker.Subscribe()`. Enables TUI and external Go clients to use the same interface.                                                  |
...
- **Standardized action keys**: `n` = new, `x` = remove, `X` = kill, `j`/`k` = navigate, `s` = shell (attach to auxiliary bash session in project detail view).
