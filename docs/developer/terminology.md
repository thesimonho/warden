...
| **Access Item**    | A general-purpose credential and mount provider. Includes built-in items (Git, SSH, GPG) for common infrastructure needs and user-defined items for custom access methods. Each item has a detection mechanism to verify availability on the host.                                                                                                                                                                                                                                                                                                                                                                 | Warden     |
...
| Notifications | Warden + Agent    | Claude Code's hook events push attention state via event bus; Warden broadcasts via SSE. The system tray (`warden-tray`) subscribes to SSE and sends native desktop notifications when agents need attention, controlled by the server-side `notificationsEnabled` setting. Codex does not yet support hooks — attention tracking is a known gap. |
