...
| File          | Purpose                                                                                                                                                                                                                                                                                                                                                             |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client.go`   | `Client` interface — the key architectural boundary. Each method maps 1:1 to an API endpoint with GoDoc comment. Methods: list/add/remove projects, reset costs, purge audit, list/create/connect worktrees, terminal operations (including `AttachTerminal(ctx, projectID, agentType, worktreeID)`), settings, event log, defaults, event subscription.            |
| `adapter.go`  | `ServiceAdapter` wraps `*Warden` to satisfy `Client` for embedded mode (TUI binary). Most methods delegate to `w.Service`; `AttachTerminal(ctx, projectID, agentType, worktreeID)` uses docker exec; `SubscribeEvents` uses `w.Broker.Subscribe()`. Enables TUI and external Go clients to use the same interface.                                                  |
