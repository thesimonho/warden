...
| File             | Purpose                                                                                |
| ---------------- | -------------------------------------------------------------------------------------- |
| `client.go`      | `Client` struct, `New(baseURL)`, all API methods (projects, worktrees, containers, settings, access items, host utilities, audit log, SSE events, WebSocket terminal), `APIError` type with `StatusCode`, `Code`, and `Message` fields, `TerminalConnection` interface, HTTP/SSE/WebSocket helpers, `deleteWithBody` helper for DELETE with request body |
...
| GET    | `/api/v1/access`                                    | `handleListAccessItems`    | List all access items (user + built-in) with detection status |
...
| `ContainerFormView` | Create or edit container. Full field set matching the webapp: name, path (directory browser), skip permissions, network mode, allowed domains, and collapsible Advanced section with access item selection (auto-populated from server), image, bind mounts (user-defined), and environment variables. Split across three files for readability. |
...
| `db.go`         | SQLite schema: `projects` table (project_id, name, added_at, image, project_path, env_vars, mounts, original_mounts, skip_permissions, network_mode, allowed_domains, enabled_access_items, cost_budget), `access_items` table (id, label, description, method, credentials JSON), `settings` table (key, value), `events` table (audit events with category, source, level), `session_costs` table (project_id, session_id, cost, is_estimated, created_at, updated_at). `enabled_access_items` column stores comma-separated access item IDs. Additive migrations via ALTER TABLE (idempotent). Legacy `worktree_costs` table dropped. |
