...
| `host.go`          | `GetDefaults` (SSH agent detection, default mounts/env vars), `ListDirectories(path, includeFiles)` (dirs-first sort; file mode for bind mount browser), `RevealInFileManager`, `ListRuntimes` |
