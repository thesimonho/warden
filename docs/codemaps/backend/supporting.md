...
- `warden.go` — top-level `Warden` type: `New(Options)` wires SQLite database, runtime detection, engine client, event bus pipeline, and service layer into a ready-to-use handle; `Close()` shuts down all subsystems (idempotent via `sync.Once`). Fields: `Service` (primary interface for all operations), `Broker` (SSE event bus), `Engine` (container runtime client, advanced use), `DB` (SQLite store, advanced use), `Watcher` (file-based event watcher, advanced use). All container/worktree operations are accessed through `w.Service`. Session watcher lifecycle is managed by the service layer: `service.StartSessionWatcher()` and `service.StopSessionWatcher()` handle JSONL session file monitoring. Watchers are stopped only by explicit lifecycle events (delete, stop, rename, recreate, shutdown) — not on heartbeat stale. This is the primary entry point for Go library consumers.
