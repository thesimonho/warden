...
| `strings.go` | `MaxToolInputLength` (1000), `MaxPromptLength` (500), `TruncateString(s, maxLen)` — text truncation with ASCII fast path, `WorktreeIDFromCWD(cwd, workspaceDir)` — extracts worktree ID from container-side paths |
...
| `codex/jsonl_types.go` | `RolloutItem`, `SessionMeta`, `TurnContext`, `ResponseItem` (with `Action *ShellAction`, `Input` for custom tools), `ShellAction`, `CompactedItem`, `EventMsg` (with `NumTurns` for thread_rolled_back), `TokenCountInfo`, `RateLimits` — JSONL line types matching Codex format |
