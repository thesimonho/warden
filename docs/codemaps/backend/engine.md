...
| File | Purpose |
| --- | --- |
| `worktrees.go` | `CreateWorktree`, `ListWorktrees` (accepts `skipEnrich` flag for event-driven mode), `ConnectTerminal` (validates worktree exists in git, auto-detects background state and reconnects), `DisconnectTerminal`, `RemoveWorktree` (kills abduco, prunes git metadata, tolerates already-removed worktrees, cleans up .warden/terminals/<id>/, uses full path via `resolveWorktreePath` for `git worktree remove`), `CleanupOrphanedWorktrees` (3-step: `pruneGitWorktrees` removes orphaned git entries, `cleanupOrphanedWorktreeDirs` checks both `.claude/worktrees/` and `.warden/worktrees/`, batched removal per prefix, `cleanupStaleTerminals` checks both prefixes for terminal dirs and kills orphaned abduco sessions first), `isAbducoSessionAlive`, `isGitWorktreeKnown`; worktree discovery: git porcelain (main + linked worktrees, prunable entries skipped) → `resolveWorktreePath(ctx, containerID)` (uses `worktreesPrefixSuffix(ctx, containerID)` method to pick agent-aware prefix) → `mergeTerminalWorktrees` (pre-git-create race, agent-aware) → `enrichWorktreeState` (terminal state from `.warden/terminals/`) (receiver `ec`). `worktreeIDFromPath()` extracts ID from both `.claude/worktrees/` and `.warden/worktrees/` paths (no legacy `.worktrees/`). |
| `worktrees_test.go` | Worktree parsing tests (`parseGitWorktreeList` incl. claude path + prunable skip, `parseTerminalBatch` incl. abduco alive/dead/backwards-compat, `isValidWorktreeID`, `worktreeIDFromPath` testing both prefixes) |
