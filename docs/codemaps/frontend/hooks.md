...
| `hooks/use-terminal.ts` | xterm.js lifecycle: create/attach xterm instance, WebSocket connection to `/api/projects/{id}/ws/{wid}`, resize events, reconnect logic, cleanup on unmount. Buffers incoming WebSocket messages and flushes to xterm.js once per `requestAnimationFrame` to prevent rapid output bursts from blocking the main thread. Scrollback increased to 10,000 lines. On disconnect: serializes buffer to IndexedDB via `saveScrollback()` before cleanup. On reconnect: restores saved scrollback from IndexedDB via `loadScrollback()` before WebSocket connect. Uses `SerializeAddon` from `@xterm/addon-serialize` to capture full buffer state (colors, attributes, cursor). |
