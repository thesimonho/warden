...
func TestPersistSessionCost_WarnOnly(t *testing.T) {
	...
	svc.PersistSessionCost("test-project", "claude-code", "test-project", "session-1", 15.00, false)
...
func TestPersistSessionCost_WithinBudget(t *testing.T) {
	...
	svc.PersistSessionCost("test-project", "claude-code", "test-project", "session-1", 5.00, false)
...
func TestPersistSessionCost_NoBudget(t *testing.T) {
	...
	svc.PersistSessionCost("unknown-project", "claude-code", "unknown-project", "session-1", 50.00, false)
...
func TestPersistSessionCost_EmptySessionIDStillEnforces(t *testing.T) {
	...
	_ = svc.db.UpsertSessionCost("test-project", "claude-code", "session-1", 15.00, false)

	// Call with empty session ID — no DB write, but enforcement still runs
	// against the previously persisted data.
	svc.PersistSessionCost("test-project", "claude-code", "test-project", "", 0, false)
...
func TestPersistSessionCost_ZeroCostStillEnforces(t *testing.T) {
	...
	svc.PersistSessionCost("test-project", "claude-code", "test-project", "session-2", 0, false)
...
func TestPersistSessionCost_MultiWorktreeAggregateExceedsBudget(t *testing.T) {
	...
	svc.PersistSessionCost("test-project", "claude-code", "test-project", "session-1", 6.00, false)

	if mock.stopCalled {
		t.Error("container should not be stopped when single session is within budget")
	}

	svc.PersistSessionCost("test-project", "claude-code", "test-project", "session-2", 6.00, false)
