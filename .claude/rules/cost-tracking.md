...
All cost writes MUST go through `service.PersistSessionCost(projectID, agentType, sessionID, cost, isEstimated)` — the single gateway that persists cost to DB keyed by `(projectID, agentType)` and triggers budget enforcement (analogous to `db.AuditWriter.Write` for audit events).
